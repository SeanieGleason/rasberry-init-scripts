...
if [ -z "$(ls -A /data)" ]; then
  ...
  wget --mirror --convert-links --page-requisites --no-parent robots=off  -P ./data "$wayback_url" || true
  echo "==> Download complete."
  echo "==> Moving to nginx index.html dir."

  ls "./data/web.archive.org/web/${timestamp}id_/$file_url/"
  mv -f "./data/web.archive.org/web/${timestamp}id_/$file_url/$file_url/" "/usr/share/nginx/html"
  echo "==> Done move."
else
